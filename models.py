...
def create_modules(module_defs,device):
    ...
    for i, module_def in enumerate(module_defs):
        ...
        if module_def['type'] == 'convolutional':
            ...
            filters = sum([output_filters[i + 1 if i > 0 else i] for i in layers])      # 计算出通道数
            ...
            yolo_layer = YOLOLayer(anchors, nC, img_size, yolo_layer_count, hyperparams['cfg'], device)
        ...
        if i == 0 and skip_first_layer:
            if module_def['type'] == 'convolutional':
                ...
                if module_def['batch_normalize']:
                    ...
                    ptr += num_b
                # Load conv. weights
                num_w = int(conv_layer.weight.numel() / 8 * 3)
