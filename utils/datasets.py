...
from evaluate.anno_func import type45
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=416, augment=False,cls=45):
        self.cls = cls
    ...
    def __getitem__(self, index):
        ...
        if channel_choose:
            ...
            table2 = []
            for i in range(256):
                table1.append(((i / 255.0) ** invGamma1) * 255)
                table2.append(((i / 255.0) ** invGamma2) * 255)
            table1 = np.array(table1).astype("uint8")
            table2 = np.array(table2).astype("uint8")
            img_gamma1 = cv2.LUT(img_gray, table1)
            img_gamma2 = cv2.LUT(img_gray, table2)
            ...
            # 生成 img_bright
            img_bright = np.max(img,2)

            # 通道合成
            img = np.stack([img[:,:,0],img[:,:,1],img[:,:,2],img_gamma1,img_gamma2,img_s,img_equ,img_bright],axis=2)
            ...
            if x.size > 0:
                ...
                if self.cls == str(3):
                    for c in range(labels.shape[0]):
                        if type45[int(labels[c][0])][0]=='w':
                            labels[c][0] = 0.0
                        elif type45[int(labels[c][0])][0]=='p':
                            labels[c][0] = 1.0
                        elif type45[int(labels[c][0])][0]=='i':
                            labels[c][0] = 2.0
                elif self.cls == str(1):
                    labels[:,0] = 0.0
    ...
    def collate_fn(batch):
        img, label, path, hw = list(zip(*batch)) # transposed   batch [(),(),()]
