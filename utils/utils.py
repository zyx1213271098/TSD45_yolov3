...
import sys
sys.path.append('home/zyx/dataTSD45_yolov3/utils/focalloss.py')
...
def compute_loss(p, targets,cls=45):  # predictions, targets
    '''新增类别参数，用于区别预测类别是：1类 还是 45类'''
    ...
    FL = FocalLoss(nn.BCEWithLogitsLoss(),gamma=2.0,alpha=1)
    ...
    for i, pi0 in enumerate(p):  # layer i predictions, i
        ...
        if len(b) > 0:
            ...
            # cls loss
            if int(cls) > 1: # 如果类别数大于1类，则使用多分类的交叉熵
                lcls += (k * 1) * CE(pi[..., 5:], tcls[i])  # class_conf loss
            else:
                tcls_i = torch.ones_like(tcls[i]).float() # 标签虽为0，但为正类，概率应为1
                lcls += (k * 1) * BCE(pi[..., 5], tcls_i)  # class_conf loss

        # pos_weight = FT([gp[i] / min(gp) * 4.])
        # BCE = nn.BCEWithLogitsLoss(pos_weight=pos_weight)
        # conf loss
        lconf += (k * 64) * BCE(pi0[..., 4], tconf)  # obj_conf loss BCE
        #lconf += FL(pi0[...,4],tconf)  # Focal loss，选择权重系数。
...
def build_targets(model, targets):
    ...
    for i, layer in enumerate(get_yolo_layers(model)):
        ...
        gwh = targets[:, 4:6] * layer.nG    # 得到框在特征图上的宽和高
        iou = [wh_iou(x, gwh) for x in layer.anchor_vec]     # anchor_vec = anchor / stride
        ...
        # XY coordinates
        txy.append(gxy - gxy.floor())   #GT 与左上角的偏差，anchor的锚点
...
def non_max_suppression(prediction, conf_thres=0.5, nms_thres=0.4):
    ...
    for image_i, pred in enumerate(prediction):
        ...
        pred[:, 4] *= class_conf   # object_conf * class_conf
