...
type45 = "i2,i4,i5,il100,il60,il80,ip,io,p10,p11,p12,p19,p23,p26,p27,p3,p5,p6,pg,ph4,ph4.5,ph5,pl100,pl120,pl20,pl30,pl40,pl5,pl50,pl60,pl70,pl80,pm20,pm30,pm55,pn,pne,pr40,po,w13,w32,w55,w57,w59,wo"
type45 = type45.split(',')
...
def eval_annos(annos_gd, annos_rt, iou=0.5, imgids=None, check_type=True, types=None, minscore=40, minboxsize=0,
               maxboxsize=400, match_same=True):
    ac_n, ac_c = 0, 0
    rc_n, rc_c = 0, 0
    if imgids == None:
    ...
    for imgid in imgids:
        ...
        if types != None:
            ...
            for i in range(len(match_g)):
                if len(types)==3:
                    objs_g[i]['category'] = objs_g[i]['category'][0]+'*'
                if not objs_g[i]['category'] in types.keys():
                    match_g[i] = -2
            for i in range(len(match_r)):
                if len(types)==3:
                    objs_r[i]['category'] = objs_r[i]['category'][0]+'*'
                if not objs_r[i]['category'] in types.keys():
                    match_r[i] = -2
        for i in range(len(match_r)):
    ...
    report = "iou:%s, size:[(%s,%s), types:%s, acc:%s, rec:%s, ac_c:%s, ac_n:%s, rc_c:%s, rc_n:%s" % (
        iou, minboxsize, maxboxsize, styps, 1 if ac_n == 0 else round(ac_c * 1.0 / ac_n,3),
        1 if rc_n == 0 else round(rc_c * 1.0 / rc_n,3), ac_c, ac_n, rc_c, rc_n)
    summury = {
        "iou": iou,
    ...
    #print('total_target:',total_target,'pre_target:',pre_target)
    return summury


def get_acc_res(results_annos, annos,type=type45,**argv):
    ...
    for i, score in enumerate(np.linspace(0, scs[-1], 100)):
        sm = eval_annos(annos, results_annos, iou=0.5, check_type=True, types=type, minscore=score, **argv)
